...
'''
# example(check a sample using Lacal PC)
replay_tensor = replay_tensor[12:20,400:420,:,:,:]
del(replay)
'''
...
def embedding_preprocessing(replay_cat, features, matchs, timesteps):
    ...
    # scale definition
    if features == 0: # 'visibility_map' feature
        scale_ = 4    
    elif features == 2: # 'player_relative' feature
        scale_ = 5
    elif features == 4: # 'unit_type' feature
        scale_ = 1914  
    else:
        scale_ = 2
...
class Embedder(nn.Module):
    def __init__(self, categori_size, emb_dim):
        super().__init__()
        self.categori_size = categori_size
        self.emb_dim = emb_dim
        self.embed = nn.Embedding(categori_size, emb_dim)
...
class CNNModel(nn.Module):
    def __init__(self):
        ...
        # Convolution 4
        self.cnn4 = nn.Conv2d(in_channels=241, out_channels=120, kernel_size=(1,2), stride=1, padding=0)
        self.relu4 = nn.ReLU()
        ...
        # Convolution 5
        self.cnn5 = nn.Conv2d(in_channels=120, out_channels=120, kernel_size=(1,2), stride=1, padding=0)
        self.relu5 = nn.ReLU()
    ...
    def forward(self, x):
        ...
        # Convolution 5
        out = self.cnn5(out).requires_grad_()
        out = self.relu5(out)
...
class ConvLSTMModel(nn.Module):
    def __init__(self, categori_size, emb_dim):
        ...
        self.categori_size = categori_size
        self.emb_dim = emb_dim
        self.Embedding = Embedder(self.categori_size, self.emb_dim)
    ...
    def forward(self, replay_tensor, batch_size):
        ...
        for i in range(batch_size):
            for j in range(replay_cat.shape[1]):
                for k in range(replay_cat.shape[2]):
                    ...
                    replay_pre = torch.LongTensor(replay_pre).to(device) # (26, 500, scale_, 128*128)
                    replay_emb = self.Embedding(replay_pre).to(device)
                    ...
                    if k == 0:
                        ...
                        replay_emb_time = torch.cat([replay_emb_time, replay_emb_feat], dim=0) # replay_emb_time = (numof cat, emb_dim)
                
                replay_emb_time = replay_emb_time.unsqueeze(dim=0) # replay_emb_time size = (1, num of cat, emb_dim)
            ...
            replay_emb_match = replay_emb_match.unsqueeze(dim=0) # replay_emb_match size = (1, timesteps, num of cat, emb_dim)
            if i == 0:
                replay_emb_full = replay_emb_match
            else:
                replay_emb_full = torch.cat([replay_emb_full, replay_emb_match], dim=0) # replay_emb_full size = (matchs, timesteps, num of cat, emb_dim)
            print('=' * 80)
        ...
        for i in range(batch_size):
            for j in range(replay_scalar.shape[1]):
                ...
                replay_sca_con = torch.FloatTensor(replay_sca_con).to(device) # (feature, 128, 128)
                replay_sca_con = replay_sca_con.unsqueeze(dim=0)
                conv_conti = Continuous_conv(emb_dim)
                replay_sca = conv_conti(replay_sca_con)
            ...
            replay_sca_time = replay_sca_time.unsqueeze(dim=0) # replay_sca_time
            if i==0:
                replay_sca_full = replay_sca_time
            else:
                replay_sca_full = torch.cat([replay_sca_full, replay_sca_time], dim=0) # replay_sca_full size = (matchs, timesteps, features, emb_dim)
        
        # replay data after preprocessing(Categorical & Continous)   
        replay_all = torch.cat([replay_sca_full, replay_emb_full], dim=2) 
        replay_all = replay_all.unsqueeze(dim=3) # replay_all size = (mathcs, timestpes, one-hot features, 1, emb_dim)
        ...
        print('================Completed CNN Modeling & Ready to LSTM Modeling================')
...
categori_size = replay_tensor.shape[3] * replay_tensor.shape[4] 
#d_model = 200
emb_dim = 200

# ConvLSTM Model
model = ConvLSTMModel(categori_size, emb_dim)
